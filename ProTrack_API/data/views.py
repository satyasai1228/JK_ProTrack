...
class Reportdata(View):
    def get(self, request):
        try:
            ...
            print(sn_agg_data, "SHP Aggregated Data")

            
            psn_list = [doc["psn"] for doc in sn_agg_data if "psn" in doc]

            crns = []
            for doc in sn_agg_data:
                if "csn" in doc:
                    crns.extend([int(k) for k in doc["csn"].keys()])  

            print(crns, "Extracted CRNs from SHP")

            
            crn_serial_details = loads(dumps(
                sts.dbcursor.SerialNumbers.find(
                    {"sn": {"$in": crns}},
                    {"_id": 0,
                     "ck":0,
                     "cc":0,
            ...
            print(len(crn_serial_details))

            serial_details = loads(dumps(
                sts.dbcursor.SerialNumbers.find(
                    {"sn": {"$in": psn_list}},
                    {"_id": 0}
                )
            ))
            print(serial_details, "PSN Serial Details")
